...
void PinholeCameraModel::rectifyImage(const cv::Mat& raw, cv::Mat& rectified, int interpolation) const
{
  ...
  /*switch (cache_->distortion_state) {		//commented out so it would work with realsense.
    case NONE:
      raw.copyTo(rectified);
      break;
    case CALIBRATED:
...
cv::Point2d PinholeCameraModel::rectifyPoint(const cv::Point2d& uv_raw) const
{
  ...
  if (cache_->distortion_state == NONE || cache_->distortion_state == UNKNOWN) //unknown for realsense
    return uv_raw;
